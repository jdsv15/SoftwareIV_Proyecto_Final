    ...
    public class PacientesController : Controller
    {
        ...
        public IActionResult Expediente(int id)
        {
            var paciente = _unidadTrabajo.Paciente
                .GetAll(p => p.Id == id, includeProperties: "Medico")
                .FirstOrDefault();
            ...
            var viewModel = new ExpedienteVM
            {
                ...
                Historial = _unidadTrabajo.HistorialClinico
                    .GetAll(h => h.PacienteId == id)
                    .OrderByDescending(h => h.FechaHora),

                Archivos = _unidadTrabajo.ArchivoExpediente
                    .GetAll(a => a.PacienteId == id)
                    .OrderByDescending(a => a.FechaSubida),

                Padecimientos = _unidadTrabajo.PacientePadecimiento
                    .GetAll(p => p.PacienteId == id, includeProperties: "Padecimiento"),

                Tratamientos = _unidadTrabajo.PacienteTratamiento
                    .GetAll(t => t.PacienteId == id, includeProperties: "Tratamiento"),

                Medicamentos = _unidadTrabajo.PacienteMedicamento
                    .GetAll(m => m.PacienteId == id, includeProperties: "Medicamento")
            };

            ViewBag.CatalogoPadecimientos = new SelectList(
                _unidadTrabajo.Padecimiento.GetAll(),
                "Id",
                "Nombre");

            ViewBag.CatalogoTratamientos = new SelectList(
                _unidadTrabajo.Tratamiento.GetAll(),
                "Id",
                "Nombre");

            ViewBag.CatalogoMedicamentos = new SelectList(
                _unidadTrabajo.Medicamento.GetAll(),
                "Id",
                "Nombre");

            return View("Expediente", viewModel);
        }
        ...
        public IActionResult AgregarNotaHistorial(int pacienteId, string notas)
        {
            ...
            var paciente = _unidadTrabajo.Paciente.Get(pacienteId);

            if (paciente == null)
            {
                TempData["Error"] = "No se encontró el paciente.";
                return RedirectToAction(nameof(Index));
            }

            if (User.IsInRole("Medico") && string.IsNullOrWhiteSpace(paciente.MedicoId))
            {
                paciente.MedicoId = medicoId;
            }

            // Se actualiza la última atención del paciente.
            paciente.FechaUltimaAtencion = DateTime.Now;
        ...
        public IActionResult SubirExamen(int pacienteId, string descripcion, IFormFile archivo)
        {
            if (archivo != null && archivo.Length > 0)
            {
                ...
                if (extension == ".pdf" || extension == ".jpg" || extension == ".jpeg" || extension == ".png")
                {
                    ...
                    TempData["Success"] = "Examen guardado correctamente.";
                }
                else
                {
